...
namespace kampong_goods.Pages.Customers
{
    ...
    public class CustListModel : PageModel
    {
        ...
        public CustListModel(CustomerService customerService, UserManager<AppUser> userManager, SignInManager<AppUser> signInManager,
...
*/        }
        ...
        public async Task<IActionResult> OnGetCustomerDisable(string id)
        {
            if (id == null)
            {
                return Page();
            ...
            */
            ...
            if (user != null)
            {
                ...
                bool isCust= await userManager.IsInRoleAsync(user, "Customer");
                if (isCust)
                {
                    //Create the Disable Customer role if NOT exist
                    IdentityRole role = await roleManager.FindByNameAsync("CustomerDisabled");
                    if (role == null)
                    {
                        IdentityResult result1 = await roleManager.CreateAsync(new IdentityRole("CustomerDisabled"));
                        if (!result1.Succeeded)
                        {
                            ModelState.AddModelError("", "Disable customer role failed");

                        }
                    }

                    var removefromrole = await userManager.RemoveFromRoleAsync(user, "Customer");
                    if (removefromrole.Succeeded)
                    {
                        //Add to disabled role
                        var disable = await userManager.AddToRoleAsync(user, "CustomerDisabled");
            ...
            TempData["FlashMessage.Type"] = "danger";
            TempData["FlashMessage.Text"] = string.Format("No such customer account.");
